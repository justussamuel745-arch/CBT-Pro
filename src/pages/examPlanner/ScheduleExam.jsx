...
function validate(form) {
  ...
  if (!form.date) {
    ...
    } else if (pickedDate.getTime() < tomorrow.getTime()) {
  ...
  if (form.date && form.time && !errors.date) {
    ...
    if (isNaN(examDateTime.getTime())) {
      ...
      examDateTime = null;
    } else if (examDateTime.getTime() - Date.now() < ONE_DAY_MS) {
      errors.date = "Exam must be scheduled at least one day from now.";
      examDateTime = null;
    }
  }
...
export default function ScheduleExam() {
  ...
  const examId = isEdit ? initialValues?._id : null 
  const total = getTotalScore(form.subjects);
  const effectRun = useRef(false) // added because of strict mode
