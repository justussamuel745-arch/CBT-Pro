...
export default function ExamHistory() {
   ...
   *
   * Example:
   * "665abc..." -> that button is loading.
   *
   * null -> no reschedule request is running.
   */
  const [
    reschedulingExamId,
  ...
  useEffect(() => {
    /*
     * Only skip the request when the store actually contains
     * usable history data.
     *
     ...
     * if (history)
     *
     * would incorrectly prevent the API request.
     */
    const hasStoredData =
      statsData !== null &&
      statsData !== undefined &&
  ...
  /* ==========================================================
     Reschedule
     ========================================================== */
  const onReschedule = async (examId) => {
    /*
     * Prevent another click while the current request is
     * running.
     */
    if (
      !examId ||
    ...
    try {
      ...
      /*
       * If navigation succeeds, this component will normally
       * unmount. If it doesn't, the button is enabled again.
       */
      setReschedulingExamId(null);
    }
  };
    ...
    useMemo(() => {
      ...
      return ACHIEVEMENT_DEFINITIONS.map(
        (achievement) => ({
          ...achievement,
          unlocked:
            achievements ? achievements.includes(
              achievement.id
            ) : false,
        })
      );
